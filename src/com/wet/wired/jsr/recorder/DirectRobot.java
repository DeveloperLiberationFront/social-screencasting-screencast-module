...
public final class DirectRobot
{
	...
	private static final Logger logger = Logger.getLogger(DirectRobot.class);
	...
	public BufferedImage createScreenCapture(Rectangle recordArea)
	{
		int[] pixels = getRGBPixels(recordArea);
		ColorModel model = new DirectColorModel(32, 0xff0000, 0xff00, 0xff);
		return new BufferedImage(model, Raster.createWritableRaster(model.createCompatibleSampleModel
				(recordArea.width, recordArea.height), new DataBufferInt(pixels, recordArea.width * recordArea.height), null), false, 
